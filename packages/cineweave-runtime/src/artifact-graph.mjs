...
async function listApprovalRecords(projectRoot) {
  ...
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    if (record.kind !== "cineweave_approval_record" || !["2.2.0", "2.3.0", "2.3.1", "2.4.0", "2.5.0"].includes(record.contractVersion)) throw new Error(`Approval kind/version mismatch: ${path}`);
