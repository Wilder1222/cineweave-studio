...
function usage() { console.error("Usage: node scripts/migrate-v1-to-v1.1.mjs <input.json> --out <copy.json> --report <report.json> [--schema <schema.json>]"); }
...
async function main() {
  ...
  const sourceVersion = source.contractVersion || (/v?(\d+\.\d+\.\d+)/.exec(source?.skillReceipt?.ref || "")?.[1]) || "1.0.x-or-earlier";

  if (migrated.contractVersion !== "1.1.0") {
    migrated.contractVersion = "1.1.0";
    changes.push(`set contractVersion=1.1.0 on migrated copy (source ${sourceVersion})`);
  }
  ...
  const report = {
    kind: "cineweave_contract_migration_report", sourceVersion, targetVersion: "1.1.0", payloadKind: source.kind || "unknown",
