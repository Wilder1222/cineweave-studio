...
const expectedSkills = ["cineweave", "cineweave-character", "cineweave-scene", "cineweave-style", "cineweave-director", "cineweave-production"];
...
async function main() {
  ...
  fail(errors, expectedSkills.every((name) => skillNames.includes(name)), "manifest must include the router and all five specialists");
  const routes = [];
  ...
  for (const skill of manifest.skills || []) {
    ...
    fail(errors, index.contractPackage === "cineweave-contracts@2.0.0", `${skill.name} must point to the V2 contract package`);
  ...
  console.log(`CineWeave Studio v2 architecture passes: ${skillNames.length} Skills, ${contractKinds.size} contracts and ${routes.length} owned routes.`);
}
