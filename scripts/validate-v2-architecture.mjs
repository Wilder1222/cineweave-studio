...
async function main() {
  ...
  fail(errors, plugin.version === "2.5.0", "plugin version must be 2.5.0");
  fail(errors, manifest.suite === "cineweave-studio" && manifest.version === "2.5.0", "contract package must be CineWeave Studio v2.5.0");
  ...
  for (const skill of manifest.skills || []) {
    ...
    fail(errors, index.contractPackage === "cineweave-contracts@2.5.0", `${skill.name} must point to the V2.5 contract package`);
  ...
  console.log(`CineWeave Studio v2.5.0 architecture passes: ${skillNames.length} Skills, ${contractKinds.size} contracts and ${routes.length} owned routes.`);
}
